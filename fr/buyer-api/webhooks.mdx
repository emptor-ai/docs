...
- **`workorder.status_update`** se déclenche à chaque transition du [modèle de statut](/buyer-api/work-orders#the-status-model), pour les statuts détenus par l'un ou l'autre côté. Les modifications qui ne touchent pas `status` (un changement de priorité, une nouvelle date de complétion estimée, une réaffectation alors que le bon de travail est encore en attente de confirmation) n'en émettent pas. Quand un bon de travail passe d'un fournisseur à un autre, vous pouvez recevoir une mise à jour intermédiaire dont le `newStatus` est `Rejected`, qui clôt l'affectation précédente, suivie de la mise à jour vers le nouveau statut.
...
- **Les événements périmés sont abandonnés, pas livrés en retard.** Un événement qui n'a pas été transmis à la passerelle dans les trois heures suivant le changement est écarté. Après une panne côté OpenWrench, ou si votre endpoint est resté indisponible plus longtemps que la fenêtre de nouvelles tentatives, réconciliez en interrogeant `GET /v1/buyer/work_order/work_orders?statusChangedAt=...` sur la période manquée.
...
4. Exécutez une interrogation périodique sur `statusChangedAt` comme filet de sécurité pour tout ce que le chemin des webhooks aurait manqué.
