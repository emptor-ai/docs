...
<Warning>
**An empty work log does not mean no work happened.** WrenchMode events are recorded by the OpenWrench apps. A supplier who drives check-in and check-out through the Supplier API instead produces a service call with correct `checkInTime` and `checkOutTime` but **no work-log events and no `trueWorkTimeMillis`**.

So treat a missing `trueWorkTimeMillis` as "not measured", not as "zero hours" — fall back to the `checkOutTime` minus `checkInTime` span for those visits, and bear in mind that span includes any breaks. The same caveat applies to `checkInGeoLocation`: it is absent on remote check-ins and on visits the supplier recorded through the API without coordinates.
</Warning>
...
3. Check `checkInGeoLocation` against the location, compare `trueWorkTimeMillis` against invoiced labor hours, and review check-out notes and images. Where `trueWorkTimeMillis` is absent, use the check-in to check-out span instead.
