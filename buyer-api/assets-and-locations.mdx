...
- `fullChargeLb` accepts at most 5 decimal places and 14 total digits, so up to 9 digits before the decimal point. The extra decimals let you store ounce conversions exactly, for example `0.0625` (1 oz), `0.03125` (half an ounce), or `0.00625` (a tenth of an ounce). OpenWrench rejects values with more precision with a `400` error. It does not round them.
...
- `fullChargeLb` on an asset follows the same positivity and precision rules as on an [asset type](#asset-types).
