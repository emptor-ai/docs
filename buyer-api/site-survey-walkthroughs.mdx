...
Each walkthrough response embeds the hydrated site survey it was run against, under `siteSurvey`. The site survey carries an optional `walkthroughHeatMapRanges` field: an array of colour bands used to render walkthrough scores as a heat map.
...
- `color` — the colour to render for the band, typically a hex code.
- `lowerRange` — the lowest score that falls into the band. A band runs from its `lowerRange` up to the next band's `lowerRange`.

To colour a score from the walkthrough's `scores` array, pick the band with the highest `lowerRange` that is less than or equal to the score.
