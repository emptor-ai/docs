...
**Tag people** by sending `taggedUsers`, a list of email addresses, next to `note`. The server turns each email into a `{ "type": "user", "value": "<email>" }` entry in the note's `elements`, the structure the OpenWrench app writes for an @mention, and adds a `text` element holding the note text after them, so keep the names out of `text`. Each tagged person is notified about the note and, when the email belongs to an active OpenWrench user with access to the work order (a buyer contact at the work order's company, or a supplier contact at a facility assigned to or with visibility on the work order), is added to the work order's subscriber list so they also receive later updates. An email that matches no such contact is stored on the note but adds nobody to the subscriber list. `taggedUsers` takes up to 25 emails; duplicates are folded case-insensitively and anything that is not an email address is rejected with `400`. If you build `elements` yourself, `taggedUsers` only appends the people not already tagged. When you read the thread back, each `user` element carries the matching contact in `userObject`.
...
3. Track supplier progress through [webhooks](/buyer-api/webhooks) (`workorder.status_update`), with `GET /work_orders?statusChangedAt=...` as a polling fallback.
