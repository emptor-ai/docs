...
**Tag people** by adding `elements` to the note. `elements` is the note's structured content: `{ "type": "text", "value": "..." }` entries carry runs of plain text and `{ "type": "user", "value": "<email>" }` entries tag a person, the same as an @mention in the OpenWrench app. Keep `text` as the plain-text version of the whole note. Each tagged person is notified about the note and, when the email belongs to an active OpenWrench user with access to the work order (a buyer contact, or a supplier contact at a facility assigned to or with visibility on the work order), is added to the work order's subscriber list so they also receive later updates. An email that matches no such contact is stored on the note but adds nobody to the subscriber list. When you read the thread back, each `user` element carries the matching contact in `userObject`.
