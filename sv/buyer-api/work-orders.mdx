...
**Tagga personer** genom att skicka `taggedUsers`, en lista med e-postadresser, bredvid `note`. Servern gör om varje e-postadress till en post `{ "type": "user", "value": "<email>" }` i anteckningens `elements`, den struktur som OpenWrench-appen skriver för ett @omnämnande, och lägger därefter till ett `text`-element med anteckningens text, så håll namnen utanför `text`. Varje taggad person aviseras om anteckningen och, när e-postadressen tillhör en aktiv OpenWrench-användare med åtkomst till arbetsordern (en köparkontakt, eller en leverantörskontakt vid en anläggning som är tilldelad arbetsordern eller har insyn i den), läggs till i arbetsorderns prenumerantlista så att de även får senare uppdateringar. En e-postadress som inte matchar någon sådan kontakt sparas i anteckningen men lägger inte till någon i prenumerantlistan. En anteckning kan tagga upp till 25 personer; dubbletter slås ihop utan hänsyn till skiftläge och allt som inte är en e-postadress avvisas med `400`. Om du bygger `elements` själv lägger `taggedUsers` bara till de personer som inte redan är taggade. När du läser tillbaka tråden bär varje `user`-element den matchande kontakten i `userObject`.
