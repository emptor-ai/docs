...
**Tagga personer** genom att lägga till `elements` i anteckningen. `elements` är anteckningens strukturerade innehåll: poster av formen `{ "type": "text", "value": "..." }` bär löpande text och poster av formen `{ "type": "user", "value": "<email>" }` taggar en person, på samma sätt som ett @omnämnande i OpenWrench-appen. Behåll `text` som en ren textversion av hela anteckningen. Varje taggad person aviseras om anteckningen och, när e-postadressen tillhör en aktiv OpenWrench-användare med åtkomst till arbetsordern (en köparkontakt, eller en leverantörskontakt vid en anläggning som är tilldelad arbetsordern eller har insyn i den), läggs till i arbetsorderns prenumerantlista så att de även får senare uppdateringar. En e-postadress som inte matchar någon sådan kontakt sparas i anteckningen men lägger inte till någon i prenumerantlistan. När du läser tillbaka tråden bär varje `user`-element den matchande kontakten i `userObject`.
