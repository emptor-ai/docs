...
- **Underliggande tillgångar.** `subAssetIds` är en valfri array med ids för underliggande tillgångar som bifogas utöver huvud-`assetId`. Underliggande tillgångar är tillgångar skapade med ett `parentId`; se [Tillgångar](/buyer-api/assets-and-locations#assets).
...
| `POST .../status_update/cancelled` | `CancelledWithReason` | Avvisas med `409` medan en tekniker är incheckad, om ditt företag aktiverar avbokningslåset som beskrivs nedan. |
...
De leverantörsägda övergångarna kommer in via leverantörens egen integration eller OpenWrench-apparna; din sida observerar dem via [webhooks](/buyer-api/webhooks) eller polling (`statusChangedAt`, `statusChanges`) och agerar på de köparägda.
...
**Tagga personer** genom att skicka `taggedUsers`, en lista med e-postadresser, bredvid `note`. Servern gör om varje e-postadress till en post `{ "type": "user", "value": "<email>" }` i anteckningens `elements`, den struktur som OpenWrench-appen skriver för ett @omnämnande, och lägger därefter till ett `text`-element med anteckningens text, så håll namnen utanför `text`. Varje taggad person aviseras om anteckningen och, när e-postadressen tillhör en aktiv OpenWrench-användare med åtkomst till arbetsordern (en köparkontakt hos arbetsorderns företag, eller en leverantörskontakt vid en anläggning som är tilldelad arbetsordern eller har insyn i den), läggs till i arbetsorderns prenumerantlista så att de även får senare uppdateringar. En e-postadress som inte matchar någon sådan kontakt sparas i anteckningen men lägger inte till någon i prenumerantlistan. `taggedUsers` tar upp till 25 e-postadresser; dubbletter slås ihop utan hänsyn till skiftläge och allt som inte är en e-postadress avvisas med `400`. Om du bygger `elements` själv lägger `taggedUsers` bara till de personer som inte redan är taggade. När du läser tillbaka tråden bär varje `user`-element den matchande kontakten i `userObject`.
...
3. Följ leverantörens framsteg via [webhooks](/buyer-api/webhooks) (`workorder.status_update`), med `GET /work_orders?statusChangedAt=...` som polling-reserv.
