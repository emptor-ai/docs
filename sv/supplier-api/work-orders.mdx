---
title: "Arbeta med arbetsordrar via Supplier API"
sidebarTitle: "Arbetsordrar"
description: "Ta emot, acceptera, avvisa och driv arbetsordrar som leverantör: statusuppdateringar, reservdelar, uppskattade slutdatum, bilagor och anteckningar."
---
...
List- och räkningsslutpunkterna är till för de två tillfällen en webhook inte kan täcka, den första laddningen av arbete som redan fanns innan din slutpunkt registrerades och en periodisk kontroll av att inget har missats:
...
Standardpaginering gäller (`offset`, `limit` standard 10 max 25, `sort_by`, `order`), och varje annan frågeparameter filtrerar på entitetens indexerade kolumner. Allt är tenant-avgränsat till din anläggning. Om ett listanrop är långsamt, smalna av filtret (en status, ett `statusChangedAt`- eller `assignedAt`-fönster) istället för att försöka igen oförändrat; en ofiltrerad lista över en stor kö kan få timeout. Två saker till att komma ihåg:
