...
Varje frågeparameter annat än pagineringsparametrarna fungerar som ett likhetsfilter på entitetens kolumner (`status=approved`, `stockLocationId=42`), alltid inom ditt företags omfång.
...
- Skapanden är inte idempotenta: att försöka igen på ett timeout-adderat PO-skapande kan duplicera ordern. Använd list-slutpunkten med ett filter (till exempel på dina `receiptNumber`-liknande externa referenser) för att kontrollera innan du försöker igen.
