...
The list and count endpoints are for the two moments a webhook cannot cover. Use them for the first load of work that already existed before you registered your endpoint, and for a periodic check that nothing was missed:
...
Standard pagination applies (`offset`, `limit` default 10 max 25, `sort_by`, `order`), and any other query parameter filters on the entity's indexed columns. Everything is tenant-scoped to your facility. If a list call is slow, narrow the filter (a status, a `statusChangedAt` or `assignedAt` window) rather than retrying it unchanged. An unfiltered list over a large queue can time out. Two more things to remember:
