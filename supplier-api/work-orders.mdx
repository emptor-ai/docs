...
**The initial status is set by the server.** For a third-party supplier key, any `status` or `isSupplierInitiated` in the body is ignored. The work order is created as `SupplierInitiatedPendingApproval` with `isSupplierInitiated: true`, and the buyer's approval configuration then decides where it lands: it stays in `SupplierInitiatedPendingApproval` until a buyer approves it, or, when the buyer auto-approves supplier-initiated work orders, it is confirmed to your facility straight away as `ConfirmedByServiceProvider`. Keys that belong to a buyer's internal service team, and paying suppliers creating a work order at a location of a client they manage, keep the `status` they send (when omitted, the initial status is derived from the buyer company's configuration; an internal team lands in `AssignedToInternalTech`).
