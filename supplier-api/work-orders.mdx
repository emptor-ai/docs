...
1. Receive new work through [webhooks](/supplier-api/webhooks) (`workorder.create`), or poll the queue (`sort_by=assignedAt`) and diff on `statusChangedAt`.
