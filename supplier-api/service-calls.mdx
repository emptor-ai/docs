...
| `POST .../service_calls/status_update/check_out` | The `checkOutStatus` you send (required) — [full list](#the-checkoutstatus-values-to-send) |
...
### The `checkOutStatus` values to send

These are the outcomes a check-out is meant to produce — the same set the OpenWrench apps offer a technician in the check-out form. **Which "work complete" value applies depends on your key**: third-party suppliers hand the job to the buyer for review, while an internal service team closes its own work.

| Outcome (label in the apps) | Third-party supplier key | Internal service team key | Resulting `displayStatus` |
| --- | --- | --- | --- |
| Work Complete | `WaitingForReview` | `InternalTechWorkComplete` | `InProgress` / `Completed` |
| Quote Required | `WaitingForQuote` | — not offered | `InProgress` |
| Delayed by Third Party | `DelayedByThirdParty` | `DelayedByThirdParty` | `OnHold` |
| Work Incomplete With Reason | `WorkIncompleteWithReason` | `WorkIncompleteWithReason` | `InProgress` |
| Parts (see below) | `PartsRequested` or `PartsOnOrder` | `PartsRequested` or `PartsOnOrder` | `OnHold` |

**The parts outcome is one value, not two.** Which one applies is set by the buyer company's parts configuration: when `partsOrderingConfig.needPartsRequisitionBeforeOrdering` is enabled the visit ends in `PartsRequested` (a requisition has to be raised before anything is ordered), otherwise it ends in `PartsOnOrder`. Send the one that matches the buyer you are working for; if you don't know, read it back off the work order after a check-out from the apps.

`WaitingForReview` and `InternalTechWorkComplete` are the two *completing* outcomes — they are the only values that stamp the work order's `completedAt` (from the check-out time) and the only ones that trigger the automation below. The other outcomes leave the job open and `completedAt` untouched.

<Warning>
**The endpoint does not enforce this list.** `checkOutStatus` is matched case-insensitively against OpenWrench's full internal work-order status enum with no transition guard, so a name outside the table above is accepted and becomes the work order's status — putting the job in a state the apps, notifications and reports do not expect from a check-out. Treat the table as the contract. An unrecognised name is rejected before anything is written:
...
<Note>
**Re-sending the status the work order is already in is a no-op on the work order.** The check-out is still recorded on the service call, but the work order comes back unchanged and `completedAt` is not re-stamped.
</Note>
