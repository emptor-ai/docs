...
- **Subactivos.** `subAssetIds` es un arreglo opcional de ids de subactivos que se adjuntan junto al `assetId` principal. Los subactivos son activos creados con un `parentId`; ver [Activos](/buyer-api/assets-and-locations#assets).
...
| `POST .../status_update/cancelled` | `CancelledWithReason` | Se rechaza con `409` mientras un técnico tiene check-in activo, si tu empresa habilita el bloqueo de cancelación descrito más abajo. |
...
Las transiciones propiedad del proveedor llegan a través de la integración del propio proveedor o de las apps de OpenWrench; tu lado las observa mediante [webhooks](/buyer-api/webhooks) o por sondeo (`statusChangedAt`, `statusChanges`) y actúa sobre las que son propiedad del comprador.
...
**Etiqueta personas** enviando `taggedUsers`, una lista de direcciones de correo, junto a `note`. El servidor convierte cada correo en una entrada `{ "type": "user", "value": "<email>" }` dentro de `elements` de la nota, la estructura que la aplicación de OpenWrench escribe para una @mención, y añade después un elemento `text` con el texto de la nota, así que deja los nombres fuera de `text`. Cada persona etiquetada recibe una notificación de la nota y, cuando el correo pertenece a un usuario activo de OpenWrench con acceso a la orden de trabajo (un contacto del comprador de la empresa de la orden de trabajo, o un contacto del proveedor en una instalación asignada a la orden de trabajo o con visibilidad sobre ella), se añade a la lista de suscriptores de la orden de trabajo para que también reciba las actualizaciones posteriores. Un correo que no coincide con ningún contacto de ese tipo se guarda en la nota pero no añade a nadie a la lista de suscriptores. `taggedUsers` admite hasta 25 correos; los duplicados se combinan sin distinguir mayúsculas y cualquier valor que no sea una dirección de correo se rechaza con `400`. Si construyes `elements` por tu cuenta, `taggedUsers` solo añade a las personas que aún no estén etiquetadas. Al volver a leer el hilo, cada elemento `user` incluye el contacto correspondiente en `userObject`.
...
3. Sigue el progreso del proveedor mediante [webhooks](/buyer-api/webhooks) (`workorder.status_update`), con `GET /work_orders?statusChangedAt=...` como sondeo de respaldo.
