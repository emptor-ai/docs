...
**Etiqueta personas** añadiendo `elements` a la nota. `elements` es el contenido estructurado de la nota: las entradas `{ "type": "text", "value": "..." }` llevan fragmentos de texto plano y las entradas `{ "type": "user", "value": "<email>" }` etiquetan a una persona, igual que una @mención en la aplicación de OpenWrench. Mantén `text` como la versión en texto plano de toda la nota. Cada persona etiquetada recibe una notificación de la nota y, cuando el correo pertenece a un usuario activo de OpenWrench con acceso a la orden de trabajo (un contacto del comprador, o un contacto del proveedor en una instalación asignada a la orden de trabajo o con visibilidad sobre ella), se añade a la lista de suscriptores de la orden de trabajo para que también reciba las actualizaciones posteriores. Un correo que no coincide con ningún contacto de ese tipo se guarda en la nota pero no añade a nadie a la lista de suscriptores. Al volver a leer el hilo, cada elemento `user` incluye el contacto correspondiente en `userObject`.
