...
Los endpoints de listado y conteo son para los dos momentos que un webhook no puede cubrir, la primera carga del trabajo que ya existía antes de registrar tu endpoint y una comprobación periódica de que nada se perdió:
...
Aplica la paginación estándar (`offset`, `limit` por defecto 10, máx. 25, `sort_by`, `order`), y cualquier otro parámetro de consulta filtra sobre las columnas indexadas de la entidad. Todo está limitado por inquilino a tu instalación. Si una llamada de listado es lenta, estrecha el filtro (un estado, una ventana de `statusChangedAt` o `assignedAt`) en lugar de reintentarla sin cambios; un listado sin filtrar sobre una cola grande puede agotar el tiempo de espera. Dos cosas más a recordar:
