...
**Mentionner des personnes** en ajoutant `elements` à la note. `elements` est le contenu structuré de la note : les entrées `{ "type": "text", "value": "..." }` portent des segments de texte brut et les entrées `{ "type": "user", "value": "<email>" }` mentionnent une personne, comme une @mention dans l'application OpenWrench. Conservez `text` comme version en texte brut de toute la note. Chaque personne mentionnée est notifiée de la note et, lorsque le courriel appartient à un utilisateur OpenWrench actif ayant accès au bon de travail (un contact acheteur, ou un contact fournisseur d'un établissement assigné au bon de travail ou ayant une visibilité sur celui-ci), elle est ajoutée à la liste des abonnés du bon de travail afin de recevoir aussi les mises à jour suivantes. Un courriel qui ne correspond à aucun contact de ce type est conservé dans la note mais n'ajoute personne à la liste des abonnés. Lorsque vous relisez le fil, chaque élément `user` porte le contact correspondant dans `userObject`.
