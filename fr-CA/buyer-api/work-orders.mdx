...
- **Sous-actifs.** `subAssetIds` est un tableau optionnel d'ids de sous-actifs attachés en plus de l'`assetId` principal. Les sous-actifs sont des actifs créés avec un `parentId`; voir [Actifs](/buyer-api/assets-and-locations#assets).
...
| `POST .../status_update/cancelled` | `CancelledWithReason` | Rejetée avec `409` pendant qu'un technicien est sur place, si votre entreprise active le verrou d'annulation décrit plus bas. |
...
Les transitions appartenant au fournisseur arrivent via sa propre intégration ou les applications OpenWrench; votre côté les observe via les [webhooks](/buyer-api/webhooks) ou le sondage (`statusChangedAt`, `statusChanges`) et agit sur celles qui vous appartiennent.
...
**Mentionner des personnes** en envoyant `taggedUsers`, une liste de courriels, à côté de `note`. Le serveur transforme chaque courriel en une entrée `{ "type": "user", "value": "<email>" }` dans les `elements` de la note, la structure que l'application OpenWrench écrit pour une @mention, puis ajoute un élément `text` contenant le texte de la note ; laissez donc les noms hors de `text`. Chaque personne mentionnée est notifiée de la note et, lorsque le courriel appartient à un utilisateur OpenWrench actif ayant accès au bon de travail (un contact acheteur de l'entreprise du bon de travail, ou un contact fournisseur d'un établissement assigné au bon de travail ou ayant une visibilité sur celui-ci), elle est ajoutée à la liste des abonnés du bon de travail afin de recevoir aussi les mises à jour subséquentes. Un courriel qui ne correspond à aucun contact de ce type est conservé dans la note mais n'ajoute personne à la liste des abonnés. `taggedUsers` accepte jusqu'à 25 courriels ; les doublons sont fusionnés sans distinction de casse et toute valeur qui n'est pas un courriel est rejetée avec `400`. Si vous construisez `elements` vous-même, `taggedUsers` n'ajoute que les personnes qui ne sont pas encore mentionnées. Lorsque vous relisez le fil, chaque élément `user` porte le contact correspondant dans `userObject`.
...
3. Suivez la progression du fournisseur via les [webhooks](/buyer-api/webhooks) (`workorder.status_update`), avec `GET /work_orders?statusChangedAt=...` comme sondage de repli.
